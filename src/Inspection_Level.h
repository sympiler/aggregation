...
int buildLevelSet_CSC(size_t n, int *Lp, int *Li, int *&levelPtr,
    ...
    /* find source nodes and append in initial queue */
    std::queue<int> queue;
    for (int i = 0; i < n; i++) {
    ...
    levelptr[0] = 0;
    while (!queue.empty()) {
        levelptr[curLevel + 1] = levelptr[curLevel] + cur_n;
    ...
    levelPtr = new int[curLevel+1]();
    std::memcpy(levelPtr, levelptr, sizeof(int) * (curLevel+1));
