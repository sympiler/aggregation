...
namespace sym_lib{
 ...
 protected:
  virtual FusionDemo *construct_demo(CSR *L, CSC* L_csc, CSC *Lt_csc,
    CSR *A, CSC *A_csc,
  ...
  virtual FusionDemo *construct_demo(CSR *L, CSC* L_csc, CSC *Lt_csc,
                                     ...
                                     CSR *A1, CSC *A1_csc,
                                     double *correct_x, std::string name,
                                     PAPIWrapper *pw, int p1, int p2,
                                     int p3){ return NULLPNTR;};
 public:
  ...
  void profile(CSR *L, CSC* L_csc, CSC *Lt_csc, CSR *A, CSC *A_csc,
   ...
   for (int i = 0; i < num_iter; ++i) {
    ...
    auto *fd = construct_demo(L, L_csc, Lt_csc, A, A_csc, correct_x, name, pw, p1, p2, p3);
  ...
  void profile(CSR *L, CSC* L_csc, CSC *Lt_csc, CSR *A, CSC *A_csc,
   ...
   for (int i = 0; i < num_iter; ++i) {
    ...
    auto *pw = new PAPIWrapper(tmp, num_instances_);
    auto *fd = construct_demo(L, L_csc, Lt_csc, A, A_csc, A1,
      A1_csc, correct_x,
      name, pw, p1, p2, p3);
    fd->set_num_threads(p1);
    fd->evaluate();
