...
//                    next[tp]=curIdx;
        ...
        void groupInspectionWithChild(int *groupPtr, int *groupSet, int &ngroup, int *groupInv, int *child)
        {
//        int *child =(int *)malloc(sizeof(int)*ncol);
//        memset(child, 0, sizeof(int)*ncol);
//
...
//            int len = mP[i + 1] - mP[i];
//            child[i] = len > 1 ? mI[mP[i] + 1] : -1;
//            auto dptr=A->diaptr[i];
//            if(dptr+1<A->p[i+1]){
//                child[i]=std::min(child[i], A->i[dptr+1]);
//            }
                if(child[i]==ncol)child[i]=-1;

//            std::cout<<i<<": "<<child[i]<<std::endl;
            }
            ...
            for (int i = 0; i < ncol; ++i) {
                ...
                while  (!visited[curIdx] && chIdx==-1)
                {
                    visited[curIdx]=true;

                    if(visited[curIdx+1]){

                        next[curIdx] = curIdx+1;
                        int tp = pre[curIdx+1];
                        pre[curIdx+1]=curIdx;
                        pre[curIdx] = tp;
                        next[tp] = curIdx;
                ...
                if(chIdx!=-1 && !visited[curIdx] ){
                    ...
                    if(status[curIdx]==0 && status[chIdx]==0){
                        ...
                        int tcurIdx = curIdx+1;
                        while (child[tcurIdx]==-1 && !visited[tcurIdx])
                        {
                            visited[tcurIdx] = true;
                            next[tcurIdx-1]=tcurIdx;
                            pre[tcurIdx]= tcurIdx-1;
